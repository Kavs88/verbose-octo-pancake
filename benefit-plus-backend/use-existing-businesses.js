...
// Test data from AI - Using EXISTING business IDs
const testData = {
  deals: [
    {
  ...
  users: [
    {
      userName: "linh_nguyen_v3",
      email: "linh.nguyen.v3@email.com",
      firstName: "Linh",
      lastName: "Nguyen",
      membershipTier: "vip",
    ...
    {
      userName: "jsmith_expat_v3",
      email: "john.smith.v3@email.com",
      firstName: "John",
      lastName: "Smith",
...
async function seedTestData() {
  try {
    console.log('🌱 Starting to seed test data using EXISTING business IDs...');
    console.log(`🏢 Using existing business IDs: ${EXISTING_BUSINESS_IDS.join(', ')}`);
    
    // 1. Seed deals with existing business relationships
    await seedDeals();
    
    // 2. Seed events with existing business relationships
    await seedEvents();
...
// Run the seeder using existing businesses
seedTestData();
